...
export function Axis({ orientation, ...props }) {
...
function AxisBottom({ scale, dimensions, label, formatTick, ...props }) {
...
function AxisLeft({ scale, dimensions, label, formatTick, ...props }) {
  const domain = scale.domain();
  const range = scale.range();
  const ticks = useMemo(() => {
