...
import { getQuantiles, normSinv } from './D3Utility';
import { defaultLineStyles, LinesChart } from './LineChart';

export function StockSim({ chartType }) {
  const [stockData, setStockData] = useState([]);
  const [stockStats, setStockStats] = useState(null);

  // Projection Google
  const [stockProjData, setStockProjData] = useState([]);
  ...
  function project2020Prices(data, stats) {
    const data2019 = data.filter(
      (data) => new Date(data.Date).getFullYear() === 2019
    );
  ...
  function multiProject2020Prices(data, stats, addQuantiles) {
    const projections = [];
  ...
  useEffect(() => {
    async function loadData() {
      const csvData = await csv('./GOOG.csv');
      ...
      <div className="inline-block p-6 text-gray-800 bg-gray-50 shadow-md rounded-md">
        ...
        <div className="flex justify-evenly">
          <div className="flex flex-col px-4">
            ...
            {/* <span>{dateFormat(d?.Date, 'Mm d, yy')}</span> */}
          </div>
          <div className="flex flex-col px-4">
